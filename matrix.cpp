...
Matrix::Matrix(vector < vector <float> > initial_grid) {  // Need to Accept 2D Vectors in this form only.
    grid = initial_grid;
    rows = initial_grid.size();
    cols = initial_grid[0].size();
}
...
Matrix Matrix::matrix_addition(Matrix other) {
    ...
    if ((rows != other.getRows()) || (cols != other.getCols())) {   // Check if two Matrix are in the Same Size or NOT.
